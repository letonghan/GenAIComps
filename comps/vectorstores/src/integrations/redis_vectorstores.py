...
from .config import INDEX_NAME, KEY_INDEX_NAME, REDIS_URL, SEARCH_BATCH_SIZE
from langchain_community.vectorstores import Redis
from redis.commands.search.field import TextField
from redis.commands.search.indexDefinition import IndexDefinition, IndexType
from comps import OpeaComponent, CustomLogger, ServiceType
from utils import encode_filename, format_search_results
...
class OpeaRedisVectorstores(OpeaComponent):
    ...
    async def ingest_chunks(
        ...
        if not self.check_index_existance(self.key_index_client):
            assert self.create_index(self.key_index_client)
    ...
    async def check_file_existance(self, file_path: str) -> bool:
        ...
        try:
            self.search_by_id(self.key_index_client, doc_id)
            if logflag:
                logger.info(f"[ check file existance ] File {file_path} exists.")
            return True
        except Exception as e:
            if logflag:
